...
class UpdateB2CStockService extends AppService
{
    ...
    public function exec()
    {
        ...
        try {
            ...
            foreach ($this->payload['items'] as $item) {
                $event = $item['event'];
                $stock = $this->b2cStockRepository->search(['sku' => $item['sku']])->first();
                ...
                if (!is_null($storageBox)) {
                    ...
                    if (!in_array($prefix, $floor)) {
                        throw ValidationException::withMessages(['box' => '此貨箱只能在B2B倉 (' . $this->payload['storageBox'] . ')使用。']);
                    }
                }

                if (
                    $event === B2CStockLog::STOCK_INPUT ||
                    $event === B2CStockLog::TRANSFER_INPUT ||
                    $event === B2CStockLog::ITEM_RETURN
                ) {
                    $this->handleInput($event, $item, $stock, $workingDay, $loggedInUser);
                } else if (
                    $event === B2CStockLog::ITEM_PICK ||
                    $event === B2CStockLog::TRANSFER_OUTPUT ||
                    $event === B2CStockLog::ITEM_EOL
                ) {
    ...
    private function handleOutput(string $event, array $item, $stock, $workingDay, $loggedInUser)
    {
        if (is_null($stock)) {
            ...
            $this->appendLog(
                $item['sku'],
                $event === B2CStockLog::ADJUST ? -$item['quantity'] : $item['quantity'],
                $subtotal,
                $event,
                $item['eventKey'],
                $item['note'],
            ...
            $blocks = [];
            array_push($blocks, $this->getSlackHeader("B2C 負庫存。"));
    ...
    {
        $this->b2cStockRepository->create([
            'sku' => $sku,
    ...
    private function appendLog(
        ...
        Log::info('B2C-stock-log:'.$eventKey.'event:'.$event.',user:'.$loggedInUser.',date:'.$workingDay);
