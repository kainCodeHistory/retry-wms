...
class CreateInventoryItemsViewTable extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    {
        Schema::dropIfExists('v_inventory_items');
    }

    private function dropView(): string
    {
        return <<<SQL
            DROP VIEW IF EXISTS `v_inventory_items`;
        SQL;
    }
    ...
    private function createView(): string
    {
        return <<<SQL
            ...
            FROM `storage_boxes` INNER JOIN `storage_box_items` ON `storage_boxes`.`id` = `storage_box_items`.`storage_box_id`
                INNER JOIN `warehouses` ON `storage_boxes`.`warehouse_id` = `warehouses`.`id` AND `warehouses`.`tt_code` = 'ED02' AND `warehouses`.`is_picking_area` = 0
