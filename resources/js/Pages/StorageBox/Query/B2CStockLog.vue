          ...
          <div class="p-4 bg-white border-b border-gray-200">
            ...
            <div class="grid md:grid-cols-2 sm:grid-cols-1 gap-2">
              <div class="mt-4">
                <wx-label for="eanSku">
<template>
    <sup class="text-red-500 font-bold">*</sup>SKU
</template>
                </wx-label>
                <wx-input id="eanSku" class="block mt-1 w-full" type="text" :value="eanSku" required autofocus  @keyup="handleKeyUp"
        ...
        async getSkuLog() {
            try {
                if (this.eanSku ?.trim().length > 0) {
                    ...
                    console.log(params)
                    const result = await this.axios.get(
                        `/api/stock-logs/${this.eanSku.trim().toUpperCase()}`, { params }
                    )
